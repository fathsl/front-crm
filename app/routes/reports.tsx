...
export default function Reports() {
  ...
  const loadTasks = async () => {
    try {
      const response = await fetch(
        "https://api-crm-tegd.onrender.com/api/Task"
      );
      if (response.ok) {
        ...
        if (data.columns) {
          data.columns.forEach((column: any) => {
            const status =
              statusMap[String(column.status)] || TaskStatus.Backlog;
  ...
  const loadAllTaskAssignments = async () => {
    try {
      const response = await fetch(
        "https://api-crm-tegd.onrender.com/api/Task/Assignments"
      );
      if (response.ok) {
        const data = await response.json();
  ...
  const loadUsers = async () => {
    try {
      const response = await fetch(
        "https://api-crm-tegd.onrender.com/api/User"
      );
      if (response.ok) {
        const data = await response.json();
        setUsers(data);
  ...
  const handleDrop = async (e: React.DragEvent, targetColumnId: TaskStatus) => {
    ...
    try {
      const response = await fetch(
        `https://api-crm-tegd.onrender.com/api/tasks/${draggedTask.id}/status`,
        {
          method: "PATCH",
      ...
      if (response.ok && draggedTask) {
        ...
        const taskToMove = tasks[draggedTask.sourceColumn]?.find(
  ...
  const updateTask = async (taskId: number, taskData: Partial<Task>) => {
    try {
      const response = await fetch(
        `https://api-crm-tegd.onrender.com/api/Task/${taskId}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const getTotalTasks = () => {
    return (Object.values(tasks) as Task[][]).reduce(
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  <div
                    className={`p-2 rounded-lg bg-white shadow-sm ${column.textColor}`}
                  >
                    {column.icon}
                  </div>
                  <div>
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`inline-flex items-center justify-center w-8 h-8 rounded-full bg-white shadow-sm text-sm font-bold ${column.textColor}`}
                  >
