  ...
  const fetchParticipants = async (meetingId: number) => {
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/Meeting/${meetingId}/participants`
      );
      if (response.ok) {
        const data = await response.json();
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                Toplantılarınızı yönetin ve detaylarını görüntüleyin
              </p>
            </div>
            <button
              onClick={() => {
                  ...
                  <div className="p-4 border-b border-gray-100">
                    <div className="flex justify-between items-start gap-3 mb-2">
                      ...
                      <div className="flex flex-col justify-end gap-2">
                        ...
                        <span
                          className={`flex items-center gap-1 px-2 py-1 rounded-md text-xs font-medium whitespace-nowrap ${getStatusStyle(meeting.status)}`}
                        >
