...
export default function Clients() {
  ...
  const [selectedUserForDiscussion, setSelectedUserForDiscussion] =
  ...
  const fetchResources = async (clientId: number) => {
    ...
    try {
      const response = await fetch(
        `${baseUrl}/api/clients/${clientId}/resources`
      );
  ...
  const fetchClientUsers = async (clientsList: any[]) => {
    try {
      ...
      const userMap = Object.fromEntries(
        users.filter(Boolean) as [string, any][]
      );
  ...
  const uploadResourcesForClient = async (
    ...
    console.log(
      `Starting upload of ${resources.length} resources for client ${clientId}`
    );
    ...
    const uploadPromises = resources.map(async (resource, index) => {
      ...
      if (resource.file) {
        ...
        console.log(
          `Resource ${index + 1}: Uploading file - ${resource.file.name}`
        );
        ...
        console.warn(
          `Resource ${index + 1}: No file or audio provided`,
          resource
        );
        return null;
      }
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.message || `Failed to upload: ${resource.title}`
          );
        }
  ...
  const createClient = async (clientData: Client) => {
    try {
      ...
      console.log(
        "Creating client with pending resources:",
      ...
      if (pendingResources.length > 0) {
        console.log(
          `Uploading ${pendingResources.length} resources for client ${clientId}`
        );
  ...
  const updateClient = async (clientId: number, userData: Client) => {
    try {
      ...
      console.log(
        "Updating client with pending resources:",
        pendingResources.length
      );
      ...
      if (pendingResources.length > 0) {
        console.log(
          `Uploading ${pendingResources.length} new resources for client ${clientId}`
        );
  ...
  const handleSubmit = async (data: FormDataType) => {
    ...
    const clientData: Client = {
      ...
      modifiedAt: new Date(),
      createdAt:
        modalMode === "add" ? new Date() : formData.createdAt || new Date(),
  ...
  const createDiscussion = async (
    ...
    selectedUserForClient?: User
  ) => {
    const isDirectCreate = directClientId !== undefined;

    if (!isDirectCreate && (!selectedUserForDiscussion || isCreatingDiscussion))
      return;
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (
          ...
          const senderLabel =
            users.find((u) => u.userId === newDiscussion.senderId)?.fullName ||
          ...
          <div className="w-full">
            <div className="space-y-4">
              {filteredClients.map((client) => (
                <div
                  key={client.id}
                  className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6 hover:shadow-lg hover:border-gray-200 transition-all duration-300"
                >
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-xs">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium text-gray-900">
                          {usersById[client.createdBy]?.fullName ??
                            "Unknown User"}
                        </span>
                      </div>
                      ...
                      {client.modifiedBy && (
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium text-gray-900">
                            {usersById[client.modifiedBy]?.fullName ??
                              "Unknown User"}
                          </span>
                        </div>
                      )}
