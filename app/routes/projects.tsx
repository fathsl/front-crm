import { AlertCircle, Calendar, CheckCircle, Clock, Edit, ListTodo, PauseCircle, PlayCircle, Plus, Save, Trash2, User, X } from 'lucide-react';
...
export default function ProjectsPage() {
  ...
  const createProject = async (projectData : Project) => {
    ...
    const handleSubmit = async () => {
        if (!formData.title || !formData.details || !formData.startDate || !formData.endDate || !formData.EstimationTime) {
        ...
        if (result.success) {
          ...
          resetForm();
          alert(modalMode === 'add' ? 'Project created successfully' : 'Project updated successfully');
        } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
                  ...
                  <div className="space-y-2">
                    {project.EstimationTime && (
                      <div className="flex items-center text-xs text-gray-500">
                        ...
                        <span className="truncate">{project.EstimationTime}</span>
                      </div>
                    )}
                    ...
                    <div className="flex items-center text-xs text-gray-500">
                      ...
                      <span className="truncate">User ID: {project.createdByUserId}</span>
                    </div>
                  </div>
      ...
      {isAddModalOpen && (
        <>
        <div className="fixed inset-0 bg-opacity-20 z-40" onClick={() => closeAddModal()}></div>
        ...
        <div className="fixed inset-y-0 right-0 w-full sm:w-96 bg-white shadow-2xl z-50 transform transition-transform duration-300 ease-in-out">
          ...
          <div className="flex flex-col h-full">
            <div className="flex-1 overflow-y-auto p-4 space-y-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Estimation Time</label>
                <input
                  type="text"
                  required
                  value={formData.EstimationTime}
                  onChange={(e) => setFormData({...formData, EstimationTime: e.target.value})}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-base transition-colors"
