  ...
  const fetchUserData = async () => {
    try {
      ...
      if (clientsResponse.ok) {
        ...
        userClients = clientsData.filter(
          (c: Client) => c.createdBy === parseInt(id || "0")
        );
        setStats((prev) => ({ ...prev, clients: userClients.length }));
      }

      const discussionsResponse = await fetch(
        `${baseUrl}/api/Chat/discussions/${id}`
      );
      ...
      if (meetingsResponse.ok) {
        ...
        userMeetings = meetingsData.filter(
          (m: Meeting) => m.clientId === parseInt(id || "0")
        );
        setStats((prev) => ({ ...prev, meetings: userMeetings.length }));
      }

      const unreadResponse = await fetch(
        `${baseUrl}/api/Chat/users/unreadcounts`
      );
      if (unreadResponse.ok) {
        const unreadData = await unreadResponse.json();
        setStats((prev) => ({
          ...prev,
          unreadMessages: unreadData[id || "0"] || 0,
        }));
      }
