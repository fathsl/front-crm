import { AlertCircle, ArrowLeft, Calendar, CheckCircle2, Clock, EyeIcon, EyeOff, FileDown, Filter, MessageSquare, UserIcon, Zap } from "lucide-react";
...
const DiscussionReportsPage: React.FC = () => {
    ...
    const statusConfig = {
      0: { label: 'Todo', color: 'bg-gray-100', borderColor: 'border-gray-300', textColor: 'text-gray-700', badge: 'bg-gray-500', icon: AlertCircle },
      1: { label: 'In Progress', color: 'bg-blue-50', borderColor: 'border-blue-300', textColor: 'text-blue-700', badge: 'bg-blue-500', icon: Zap },
      2: { label: 'Completed', color: 'bg-green-50', borderColor: 'border-green-300', textColor: 'text-green-700', badge: 'bg-green-500', icon: CheckCircle2 },
    };
    ...
    const groupedByStatus = filteredDiscussions.reduce<Partial<DiscussionsByStatus>>((acc, discussion) => {
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {discussion.senderName && (
              <div className="flex items-center gap-1 bg-white bg-opacity-50 px-2 py-1 rounded text-xs">
                ...
                <span className="text-gray-700 font-medium">{discussion.senderName}</span>
              </div>
            )}
            {discussion.receiverName && (
              <div className="flex items-center gap-1 bg-white bg-opacity-50 px-2 py-1 rounded text-xs">
                <UserIcon className="w-3 h-3 text-purple-600" />
                <span className="text-gray-700 font-medium">{discussion.receiverName}</span>
              </div>
            )}
          </div>
          {discussion.creatorName && (
            <div className="text-xs text-gray-500 mt-1">
              Created by <span className="font-medium text-gray-700">{discussion.creatorName}</span>
            </div>
          )}
        </div>
        ...
        <div className="mt-2 flex items-center gap-1">
          {discussion.isSeen ? (
            ...
            <>
              ...
              <span className="text-xs text-orange-600 font-medium">Unseen</span>
            </>
          )}
        </div>
      </div>
    ...
    const StatusColumn = ({ status , discussions }: { status: DiscussionStatus; discussions: Discussion[] }) => {
    const config = statusConfig[status];

    return (
      <div className="flex flex-col">
        <div className={`${config.color} px-4 py-3 rounded-t-lg border-b-2 ${config.borderColor}`}>
          <div className="flex items-center gap-2 mb-2">
            <div className={`${config.badge} text-white rounded-full px-2.5 py-0.5 text-xs font-semibold`}>
              {discussions.length}
            </div>
        ...
        <div className={`flex-1 p-3 space-y-3 rounded-b-lg border-2 ${config.borderColor} min-h-96 bg-white overflow-y-auto`}>
              ...
              <div className="relative flex-shrink-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium hidden sm:inline">
                    {selectedFilter === 'all' ? 'All Statuses' : getStatusText(selectedFilter)}
                  </span>
                </button>
                <button className="flex items-center gap-2 px-3 py-2 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors" onClick={() => handleExportDiscussions()}>
          ...
          {discussions.length === 0 ? (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                {Object.entries(groupedByStatus).map(([statusKey, discussions]) => (
                  <StatusColumn
                    key={statusKey}
                ...
                >
                  <div className="p-6">
                    ...
                    <div className="flex items-center gap-2 mb-4 flex-wrap">
                      <div className={`inline-block px-3 py-1 rounded-full text-xs font-semibold text-white ${statusConfig[selectedTask.status].badge}`}>
                    ...
                    <div className="mb-6 p-4 bg-gray-50 rounded-lg">
                      <h3 className="text-xs font-bold text-gray-700 mb-3 uppercase tracking-wide">Participants</h3>
                      <div className="space-y-2">
                        {selectedTask.senderName && (
                          <div className="flex items-center gap-2">
                            <UserIcon className="w-4 h-4 text-blue-600" />
                            <span className="text-sm text-gray-700">
                              <span className="font-semibold">Sender:</span> {selectedTask.senderName}
                            </span>
                          </div>
                        )}
                        {selectedTask.receiverName && (
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm text-gray-700">
                              <span className="font-semibold">Receiver:</span> {selectedTask.receiverName}
                            </span>
                          </div>
                        )}
                        {selectedTask.creatorName && (
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm text-gray-700">
                              <span className="font-semibold">Created by:</span> {selectedTask.creatorName}
                            </span>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex items-center gap-2">
                        {selectedTask.isSeen ? (
                          <>
                            ...
                            <span className="text-green-600 font-medium">Seen by recipients</span>
                          </>
                        ) : (
                          <>
                            <EyeOff className="w-4 h-4 text-orange-600" />
                            <span className="text-orange-600 font-medium">Not seen by recipients</span>
                          </>
                        )}
                      </div>
                    </div>
