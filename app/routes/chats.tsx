...
const ChatApplication: React.FC = () => {
  ...
  const [taskDrawerType, setTaskDrawerType] = useState<MessageType | null>(
    null
  );
  ...
  const [newDiscussionStatus, setNewDiscussionStatus] =
    useState<DiscussionStatus>(DiscussionStatus.NotStarted);
  const [updatingDiscussions, setUpdatingDiscussions] = useState<Set<number>>(
    new Set()
  );
  ...
  const [assignedUsers, setAssignedUsers] = useState<{
    [discussionId: number]: AssignedUser[];
  }>({});
  const [loadingAssignedUsers, setLoadingAssignedUsers] = useState<{
    [discussionId: number]: boolean;
  }>({});
  ...
  const [editingDiscussionId, setEditingDiscussionId] = useState<number | null>(
    null
  );
  ...
  const [taskMediaRecorder, setTaskMediaRecorder] =
  ...
  const getPriorityFromNumber = (priorityNum: number): TaskPriority => {
    ...
    return (
      priorityMap[priorityNum as keyof typeof priorityMap] ||
      TaskPriority.Medium
    );
  };
    ...
    async (discussionId: number) => {
      ...
      try {
        const response = await fetch(
          `${baseUrl}/api/Chat/discussions/${discussionId}/assigned-users`
        );
        if (response.ok) {
          ...
          const assignedUserIds = new Set(
            data.map((user) => user.assignedUserId)
          );
  ...
  const fetchDiscussions = useCallback(
    async (currentUserId: number, selectedUserId?: number) => {
        ...
        if (response.ok) {
          ...
          const uniqueDiscussions = data.filter(
            (discussion: Discussion, index: number, self: Discussion[]) =>
              index ===
              self.findIndex((d: Discussion) => d.id === discussion.id)
          );
            ...
            uniqueDiscussions.map(async (d) => {
              try {
                const resp = await fetch(
                  `${baseUrl}/api/Chat/discussions/${d.id}/messages?userId=${currentUser?.userId || 0}`
                );
          ...
          if (messages && messages.length > 0) {
            const latestMessage = messages.reduce(
              (latest: Message, msg: Message) =>
                new Date(msg.createdAt ?? 0) > new Date(latest.createdAt ?? 0)
                  ? msg
                  : latest
            );
  ...
  const createDiscussion = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (
          currentUser &&
          newDiscussion?.receiverId &&
          Number(newDiscussion.receiverId) === Number(currentUser.userId)
        ) {
          const senderLabel =
            users.find((u) => u.userId === newDiscussion.senderId)
  ...
  const handleStatusUpdate = useCallback(
    async (
      ...
      try {
        const response = await fetch(
          `${baseUrl}/api/Chat/discussions/${discussionId}/status`,
          {
            method: "PUT",
        ...
        if (response.ok) {
          ...
          if (currentUser && updatedDiscussion) {
            const receiverId =
              updatedDiscussion.senderId === currentUser.userId
          ...
          if (selectedDiscussion?.id === discussionId) {
            const updatedDiscResponse = await fetch(
              `${baseUrl}/api/Chat/discussions/${currentUser?.userId}`
            );
            if (updatedDiscResponse.ok) {
              const allDiscussions: Discussion[] =
                await updatedDiscResponse.json();
              const updatedDisc = allDiscussions.find(
                (d) => d.id === discussionId
              );
              if (updatedDisc) {
                setSelectedDiscussion((prev) =>
                  prev
                    ? {
                        ...prev,
                        status: nextStatus,
                        updatedAt: updatedDisc.updatedAt,
                      }
                    : prev
                );
              }
  ...
  const assignUsersToDiscussion = async (
    discussionId: number,
    userIds: number[]
  ) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.totalAssigned > 0) {
          console.log(
            `Successfully assigned ${result.totalAssigned} users to the discussion`
          );
        }

        if (result.totalSkipped > 0) {
          console.log(
            `${result.totalSkipped} users were already assigned to this discussion`
          );
        }
        ...
        console.error(
          "You don't have permission to assign users to this discussion"
        );
      } else {
  ...
  const filteredTaskClients = clients.filter(
    (client) =>
      `${client.first_name} ${client.last_name}`
        .toLowerCase()
        .includes(clientSearchTerm.toLowerCase()) ||
  ...
  const getDiscussionClientFirstNames = useCallback(
    (discussion: any): string[] => {
      try {
        ...
        return ids
          .map((id) => lookup.get(id)?.first_name)
  ...
  const filteredDiscussions = useMemo(() => {
    ...
    return discussions.filter((d) => {
      ...
      const clientNames = getDiscussionClientFirstNames(d).map((n) =>
        n.toLowerCase()
      );
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch(
        `${baseUrl}/api/Chat/messages/send-with-file`,
        {
          method: "POST",
          body: formData,
        }
      ...
      if (selectedDiscussion && currentUser) {
        try {
          await fetch(
            `${baseUrl}/api/Chat/discussions/${selectedDiscussion.id}/update-timestamp`,
            {
              method: "PUT",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
      ...
      setMessages((prev) =>
        sortByCreatedAtAsc([...(prev as Message[]), message as Message])
      );
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (
          currentUser &&
          result?.receiverId &&
          Number(result.receiverId) === Number(currentUser.userId)
        ) {
          const senderLabel =
            users.find((u) => u.userId === result.senderId)?.kullaniciAdi ||
            currentUser.kullaniciAdi;
          showToastForMessage(
            {
              ...result,
        ...
        setMessages((prev) =>
          sortByCreatedAtAsc(
            (prev as Message[]).map((msg) =>
              msg.id === messageId ? (updatedMessage as Message) : msg
            )
          )
        );
  ...
  const deleteMessage = async (messageId: number) => {
    try {
      const response = await fetch(
        `${baseUrl}/api/Chat/messages/${messageId}?userId=${currentUser?.userId || 0}`,
        {
          method: "DELETE",
        }
      );
  ...
  const deleteDiscussion = async (discussionId: number) => {
    try {
      const response = await fetch(
        `${baseUrl}/api/Chat/discussions/${discussionId}`,
        {
          method: "DELETE",
        }
      );
  ...
  const markDiscussionMessagesAsSeen = async (
    discussionId: number,
    userId: number
  ) => {
    try {
      const response = await fetch(
        `${baseUrl}/api/Chat/discussions/${discussionId}/mark-all-seen?userId=${userId}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!response.ok) {
        const errorText = await response.text();
        throw new Error(
          `Failed to mark discussion messages as seen: ${response.statusText} - ${errorText}`
        );
      }
  ...
  useEffect(() => {
    ...
    if (!currentUserId || filteredUsers.length === 0 || currentView !== "users")
      return;
        ...
        for (const u of toProcess) {
          ...
          try {
            ...
            const unique = Array.isArray(data)
              ? Array.from(
                  new Map((data as any[]).map((d: any) => [d.id, d])).values()
                )
              : [];
            const uid = Number(u.userId);
            let count = unique.filter(
              (d: any) =>
                Number(d.senderId) === uid || Number(d.receiverId) === uid
            ).length;
            ...
            for (const d of remaining) {
              ...
              try {
                const r = await fetch(
                  `${baseUrl}/api/Chat/discussions/${d.id}/assigned-users`,
                  { signal: controller.signal }
                );
                if (r.ok) {
                  const assigned: AssignedUser[] = await r.json();
                  if (
                    assigned.some((au) => Number(au.assignedUserId) === uid)
                  ) {
                    count += 1;
                  }
                }
                ...
                console.log(
                  "Error fetching assigned users for discussion:",
                  d.id
                );
              }
            }
  ...
  useEffect(() => {
    ...
    if (
      assignedUsers[selectedDiscussionId] &&
      assignedUsers[selectedDiscussionId].length > 0
    )
      return;
  ...
  const handleDiscussionSelect = useCallback(
  ...
  const sendVoiceMessage = async () => {
    ...
    try {
      ...
      formData.append(
        "fileReference",
        `voice_ref_${Date.now()}_${voiceFileName}`
      );

      console.log("Sending voice message with duration:", recordingTime);

      const response = await fetch(
        `${baseUrl}/api/Chat/messages/send-with-voice`,
        {
          method: "POST",
          body: formData,
        }
      ...
      if (
        currentUser &&
        newVoiceMessage.receiverId &&
        Number(newVoiceMessage.receiverId) === Number(currentUser.userId)
      ) {
        const senderLabel =
          users.find((u) => u.userId === newVoiceMessage.senderId)
            ?.kullaniciAdi || currentUser.kullaniciAdi;
        showToastForMessage(
          {
            ...newVoiceMessage,
      ...
      refreshDiscussionsAfterMessage();
      setMessages((prev) =>
        sortByCreatedAtAsc([...(prev as Message[]), newVoiceMessage])
      );
      setAudioBlob(null);
      setRecordingTime(0);
    } catch (error) {
  ...
  const playVoiceMessage = async (message: Message) => {
    ...
    try {
      ...
      if (message.audioBlob) {
        ...
        const response = await fetch(
          `${baseUrl}/api/Chat/messages/${message.id}/voice`,
          {
            method: "GET",
          }
        );
      ...
      alert(
        error instanceof Error ? error.message : "Failed to play voice message"
      );
      audioRef.current = null;
    }
  };
  ...
  const downloadFile = async (messageId: number, fileName: string) => {
    try {
      const response = await fetch(
        `${baseUrl}/api/Chat/messages/${messageId}/file`,
        {
          method: "GET",
        }
      );
        ...
        for (const d of slice) {
          ...
          try {
            const cResp = await fetch(
              `${baseUrl}/api/Chat/discussions/${d.id}/unreadcount?userId=${currentUser.userId}`,
              { signal: controller.signal }
            );
            ...
            if (unread > prev) {
              const mResp = await fetch(
                `${baseUrl}/api/Chat/discussions/${d.id}/messages?userId=${currentUser.userId}`,
                { signal: controller.signal }
              );
              if (!mResp.ok) continue;
              const payload = await mResp.json();
              const arr: Message[] = Array.isArray(payload?.messages)
                ? payload.messages
              ...
              for (const msg of sortedMsgs) {
                ...
                if (
                  ...
                  const senderName = String(
                    users.find((u) => u.userId === msg.senderId)
                      ?.kullaniciAdi ||
  ...
  const handleSendTask = async (
    ...
    try {
      ...
      if (taskDrawerType === MessageType.File && taskFile) {
        const formData = new FormData();
        (
          Object.keys(taskMessageData) as (keyof typeof taskMessageData)[]
        ).forEach((key) => {
        ...
        response = await fetch(
          `${baseUrl}/api/Chat/messages/send-task-with-file`,
          {
            method: "POST",
            body: formData,
          }
        ...
        response = await fetch(
          `${baseUrl}/api/Chat/messages/send-task-with-voice`,
          {
            method: "POST",
            body: formData,
          }
          ...
          showToastForMessage(
            {
              ...
              senderName: senderLabel,
              duration:
                taskDrawerType === MessageType.Voice
                  ? taskRecordingTime
                  : undefined,
            },
            senderLabel
  ...
  const createClient = async (clientData: Client) => {
    try {
      ...
      return {
        success: false,
        message: "Kullanıcı oluşturulurken hata oluştu - Network error",
      };
    }
  };
  ...
  const playTaskVoiceMessage = async (
    taskId: number,
    messageId: number,
    message: Message
  ) => {
    ...
    try {
      ...
      if (message.audioUrl) {
        ...
        const response = await fetch(
          `${baseUrl}/api/Chat/messages/${messageId}/voice`,
          {
            method: "GET",
          }
        );
  ...
  const FileMessage = ({ message }: { message: Message }) => {
    ...
    return (
      <div
        className={`p-3 rounded-lg border ${message.senderId === currentUser?.userId ? "bg-blue-50 border-blue-200" : "bg-white border-gray-200"} w-full`}
      >
  ...
  const handleDownloadExcel = async (
    ...
    try {
      const response = await fetch(
        `${baseUrl}/api/Chat/discussions/${discussionId}/tasks/export-excel`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
    ...
    try {
      ...
      if (allSuccessful) {
        ...
        const failedIds = results
          .map((response, index) =>
            !response.ok ? Array.from(selectedMessages)[index] : null
          )
          .filter((id) => id !== null);
  ...
  return (
    <div className="h-screen bg-gray-50 flex flex-col">
      ...
      <div
        className={`${currentView === "users" ? "flex" : "hidden"} w-full bg-white flex-col`}
      >
            ...
            >
              <div className="flex items-center space-x-3">
                ...
                <div className="ml-auto flex items-center">
                  <span
                    className="inline-flex items-center justify-center min-w-[24px] h-6 px-2 rounded-full text-xs font-medium bg-blue-100 text-blue-700"
                    title="Number of discussions with this user"
                  >
      ...
      <div
        className={`${currentView === "discussions" ? "flex" : "hidden"} w-full bg-white flex-col`}
      >
        <div className="p-4 border-b border-gray-100">
          <div className="flex items-center justify-between">
            <div>
            ...
            <div className="flex items-center gap-2">
              ...
              {selectedUser && (
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() =>
                      navigate(`/discussion-reports/${selectedUser?.userId}`)
                    }
          ...
          {discussions.length === 0 ? (
            ...
            filteredDiscussions.map((discussion) => {
              ...
              return (
                ...
                >
                  <div
                    className={`absolute top-0 left-0 right-0 h-1 ${getStatusBackgroundColor(
                      discussion.lastTaskStatus,
                      false
                    )}`}
                  />
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-4">
                      <span
                        onClick={(e) =>
                          handleStatusUpdate(
                            discussion.id,
                            discussion.status || DiscussionStatus.NotStarted,
                            e
                          )
                        }
                      ...
                      {discussion.lastTaskStatus !== undefined &&
                        discussion.lastTaskStatus !== null && (
                          <span
                            className={`inline-flex items-center px-2.5 sm:px-3 py-1.5 rounded-full text-xs font-medium text-white shadow-sm ${
                              discussion.lastTaskStatus === TaskStatusBg.Backlog
                                ? "bg-slate-500"
                                : discussion.lastTaskStatus ===
                                    TaskStatusBg.ToDo
                                  ? "bg-amber-500"
                                  : discussion.lastTaskStatus ===
                                      TaskStatusBg.InProgress
                                    ? "bg-blue-500"
                                    : discussion.lastTaskStatus ===
                                        TaskStatusBg.InReview
                                      ? "bg-purple-500"
                                      : discussion.lastTaskStatus ===
                                          TaskStatusBg.Done
                                        ? "bg-emerald-500"
                  ...
                  {showUsersDrawer &&
                    selectedDiscussionId === discussion.id && (
                      <>
                        ...
                        <div
                          className={`fixed inset-0 sm:top-0 sm:right-0 sm:left-auto sm:inset-auto sm:h-screen w-full sm:w-80 md:w-96 lg:w-[28rem] bg-white shadow-2xl z-50 transform transition-all duration-300 ease-out flex flex-col ${
                            showUsersDrawer
                              ? "translate-x-0"
                              : "translate-x-full"
                          }`}
                        >
                          <div className="flex-shrink-0 flex items-start sm:items-center justify-between p-4 sm:p-6 border-b bg-gradient-to-r from-purple-500 to-purple-600 text-white min-h-[80px] sm:min-h-0">
                            <div className="flex-1 min-w-0 pr-4">
                              ...
                              <p className="text-purple-100 text-sm break-words line-clamp-2">
                                {discussion.title}
                              </p>
                            </div>
                          ...
                          <div className="flex-shrink-0 p-4 bg-gray-50 border-b">
                            <div className="relative">
                              <svg
                                className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400 flex-shrink-0"
                                fill="none"
                                stroke="currentColor"
                                viewBox="0 0 24 24"
                              >
                          ...
                          <div className="flex-1 overflow-y-auto p-4 space-y-1 min-h-0">
                            {filteredUsers.map((user) => {
                              ...
                              const isAlreadyAssigned =
                                assignedUsers[selectedDiscussionId || 0]?.some(
                                  (assignedUser) =>
                                    assignedUser.assignedUserId === user.userId
                                ) || false;
                                ...
                                <button
                                  onClick={() => {
                                    filteredUsers.forEach((user) => {
                                      setSelectedUsers(
                                        (prev) => new Set(prev.add(user.userId))
                                      );
                                    });
                                  }}
      ...
      <div
        className={`${currentView === "chat" ? "flex" : "hidden"} w-full flex-col bg-gray-50 h-screen`}
      >
                  ...
                  {message.messageType === 4 ? (
                    <TaskMessage
                      task={{
                        ...
                        status:
                          (message.taskStatus as TaskStatus) ||
                          TaskStatus.Backlog,
                        priority:
                          typeof message.taskPriority === "number"
                            ? getPriorityFromNumber(message.taskPriority)
                            : (message.taskPriority as TaskPriority) ||
                              TaskPriority.Medium,
                        dueDate: message.dueDate
                          ? typeof message.dueDate === "string"
                            ? message.dueDate
                        ...
                        taskStatus:
                          ...
                          TaskStatus.Backlog,
                        taskPriority:
                          message.taskPriority || TaskPriority.Medium,
                      ...
                      onStatusChange={async (newStatus) => {
                        ...
                        try {
                          const response = await fetch(
                            `${baseUrl}/api/Task/${message.taskId}/status`,
                            {
                              method: "PUT",
                              headers: { "Content-Type": "application/json" },
                              body: JSON.stringify({
                            ...
                            if (selectedDiscussion && currentUser) {
                              try {
                                ...
                                if (!timestampResponse.ok) {
                                  console.error(
                                    "Failed to update discussion timestamp:",
                                    await timestampResponse.text()
                                  );
                                }
                              } catch (error) {
                                console.error(
                                  "Error updating discussion timestamp:",
                                  error
                                );
                              }
                            }
                      ...
                      const checks = (
                        <div className="mt-1 text-xs flex items-center space-x-1">
                          ...
                          >
                            <Check
                              size={12}
                              className="inline mr-0.5 align-text-bottom"
                            />
                            <Check
                              size={12}
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl">
            ...
            <div className="space-y-4">
                  ...
                  <div className="flex-1 flex flex-wrap gap-2">
                    {selectedClients.length === 0 ? (
                      <span className="text-slate-500 text-sm">
                        {t("chats.newDiscussion.selectClientsPlaceholder")}
                      </span>
                    ) : (
                    ...
                    <div className="p-3">
                      <div className="relative mb-3">
                        <input
                          type="text"
                          placeholder={t(
                            "chats.newDiscussion.searchClientsPlaceholder"
                          )}
                      ...
                      <div className="flex items-center justify-between mb-2 pb-2 border-b border-slate-200">
                        <span className="text-sm font-medium text-slate-700">
                          {t("chats.newDiscussion.selectClientsHeader")}
                        </span>
                        <div className="flex gap-2">
                          {filteredClients.length > 0 && (
                            <button
                              onClick={() => {
                                const newClients = filteredClients.filter(
                                  (client) =>
                    ...
                    <div className="max-h-40 overflow-y-auto px-3 pb-3">
                      ...
                      {filteredClients.length === 0 && clientSearchTerm && (
                        <div className="text-center text-slate-500 text-sm py-4">
                          ...
                          {t("chats.newDiscussion.noClientsFound", {
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setNewDiscussionStatus(
                      Number(e.target.value) as DiscussionStatus
                    )
                  }
                  className="w-full p-4 border border-slate-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition"
                >
                  <option value={DiscussionStatus.NotStarted}>
                    {t("chats.newDiscussion.status.notStarted")}
                  </option>
                  <option value={DiscussionStatus.InProgress}>
                    {t("chats.newDiscussion.status.inProgress")}
                  </option>
                  <option value={DiscussionStatus.Completed}>
                    {t("chats.newDiscussion.status.completed")}
                  </option>
                </select>
              </div>
            </div>
            ...
            <div className="flex flex-row justify-between items-center gap-2 mt-6">
              <button
                onClick={() =>
                  isEditMode ? updateDiscussion() : createDiscussion()
                }
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1">
                        <button
                          ...
                          className="p-2 text-slate-500 hover:text-slate-700 rounded-lg hover:bg-slate-200 border border-2 border-slate-300 transition"
                        >
                    ...
                    {!taskAudioBlob ? (
                      ...
                      <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                        <div className="flex items-center justify-center space-x-3">
                          ...
                          <span className="font-medium">
                            Recording ready: {formatTime(taskRecordingTime)}
                          </span>
                          <button
                            onClick={() => {
                              setTaskAudioBlob(null);
            ...
            <div className="border-t border-slate-200 p-4">
              <div className="flex justify-between">
                ...
                <button
                  ...
                  disabled={
                    (!taskContent.trim() && !taskFile && !taskAudioBlob) ||
                    isSendingTask
                  }
