import { Eye, Facebook, Heart, MessageCircle, Share2, TrendingUp, UsersIcon } from "lucide-react";
import { useEffect, useState } from "react";

export default function SocialMediaDashboard() {
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState('');
    const FB_ACCESS_TOKEN = "EAALPqqyYnVUBPpuopWdyr5YdZCY8rIkHvkwhAdU94DKHbkJlaxMrI1TADOLCIxzJy437ZAe66lzvHBb7mo72c6mYxmJCwNRlHZBSld8xAvoajziIyKsSdWi7I0Y1OPGA6g550XufSYCDbc47lo1v4CBo6rxMcE0R0SRT8Fi4vtq5Lxb56Ol96vXZAJSO64u8";
    const PAGE_ID = '';
    ...
    const initializeFacebook = async () => {
      if (!FB_ACCESS_TOKEN || FB_ACCESS_TOKEN === 'EAALPqqyYnVUBPpuopWdyr5YdZCY8rIkHvkwhAdU94DKHbkJlaxMrI1TADOLCIxzJy437ZAe66lzvHBb7mo72c6mYxmJCwNRlHZBSld8xAvoajziIyKsSdWi7I0Y1OPGA6g550XufSYCDbc47lo1v4CBo6rxMcE0R0SRT8Fi4vtq5Lxb56Ol96vXZAJSO64u8') {
    ...
    const fetchAvailablePages = async () => {
      try {
        ...
        if (data.data && data.data.length > 0) {
          ...
          setError('No Facebook Pages found. You need a Facebook Page, not a personal profile.');
          setLoading(false);
        }
      } catch (err) {
    ...
    const fetchFacebookData = async (pageId: string, pageToken?: string) => {
      try {
        ...
        const engagementRate = followers > 0 ? ((totalEngagement / postsCount) / followers * 100).toFixed(1) : '0';
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-6">
        <div className="max-w-7xl mx-auto">
          <div className="mb-8">
            ...
            <p className="text-purple-200">Monitor your social presence across all platforms</p>
          </div>
  
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-6 border border-white/20">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-white text-lg font-semibold">Total Followers</h3>
                <UsersIcon className="text-purple-300" size={24} />
              </div>
              <p className="text-4xl font-bold text-white mb-2">
                {(socialData.facebook.followers + socialData.instagram.followers + socialData.linkedin.followers).toLocaleString()}
              </p>
            ...
            <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-6 border border-white/20">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-white text-lg font-semibold">Total Engagement</h3>
                <Heart className="text-pink-300" size={24} />
              </div>
              <p className="text-4xl font-bold text-white mb-2">
                {(socialData.facebook.likes + socialData.instagram.likes).toLocaleString()}
              </p>
            ...
            <div className="bg-white/10 backdrop-blur-lg rounded-2xl p-6 border border-white/20">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-white text-lg font-semibold">Avg. Engagement</h3>
            ...
            <div className="bg-gradient-to-br from-blue-600 to-blue-800 rounded-2xl p-6 shadow-2xl transform transition hover:scale-105">
              ...
              <div className="space-y-4 mb-6">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-white font-bold text-lg">{socialData.facebook.followers.toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-blue-100">Likes</span>
                  <span className="text-white font-bold text-lg">{socialData.facebook.likes.toLocaleString()}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-blue-100">Shares</span>
                  <span className="text-white font-bold text-lg">{socialData.facebook.shares}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-blue-100">Engagement Rate</span>
                  <span className="text-white font-bold text-lg">{socialData.facebook.engagement}%</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-blue-100">Recent Posts</span>
                  <span className="text-white font-bold text-lg">{socialData.facebook.recentPosts}</span>
                </div>
              </div>
          ...
          <div className="mt-8 bg-white/10 backdrop-blur-lg rounded-2xl p-6 border border-white/20">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
              <div>
                <MessageCircle className="mx-auto text-purple-300 mb-2" size={24} />
                <p className="text-2xl font-bold text-white">{socialData.instagram.comments + 234}</p>
                <p className="text-purple-200 text-sm">Total Comments</p>
              </div>
              <div>
                <Share2 className="mx-auto text-purple-300 mb-2" size={24} />
                <p className="text-2xl font-bold text-white">{socialData.facebook.shares}</p>
                <p className="text-purple-200 text-sm">Total Shares</p>
              </div>
              <div>
                <Eye className="mx-auto text-purple-300 mb-2" size={24} />
                <p className="text-2xl font-bold text-white">{socialData.linkedin.impressions.toLocaleString()}</p>
