import { BarChart3, Cpu, FilterIcon, Globe, MapPin, Package, Phone, Search, ShoppingCart, TrendingUp, User, Users } from "lucide-react";
...
export async function loader({ request }: LoaderFunctionArgs): Promise<Response> {
    ...
    try {
      ...
      if (!productCategoriesRes.ok || !componentsRes.ok || !customerPurchasesRes.ok || 
          !countryDeliveriesRes.ok || !regionDeliveriesRes.ok || !customersRes.ok || !ordersRes.ok || !filterOptionsRes.ok) {
...
export default function Reporting() {
  ...
  const [filterOrders, setFilterOrders] = useState<FilteredOrderDto[]>(data?.filterOptions?.orders || []);
  ...
  const applyFilters = async () => {
    try {
      const response = await fetch(`${process.env.API_BASE_URL}/api/reporting/orders/filter`, {
        method: 'POST',
        headers: {
        ...
        <div className="max-w-7xl mx-auto px-4 py-4 sm:px-6 sm:py-6">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div key={index} className="bg-white rounded-lg p-4 shadow-sm">
                    ...
                    <div>
                        <p className="text-xs font-medium text-gray-600 sm:text-sm">{card.title}</p>
                ...
                {data?.productCategories.map((category, index) => (
                    ...
                    <div className="flex justify-between items-start">
                        ...
                        <div className="mt-2 flex items-center">
                            ...
                            category.stock > 100 ? 'bg-green-100 text-green-800' :
                            category.stock > 50 ? 'bg-yellow-100 text-yellow-800' :
            ...
            <div className="space-y-3">
                ...
                {data?.components.map((component) => (
                    <div key={component.componentId} className="bg-white rounded-lg shadow-sm p-4">
                    <div className="flex justify-between items-start">
                        <div>
                        <h4 className="font-medium text-gray-900">{component.componentName}</h4>
                        <div className="mt-1 text-xs text-gray-500">ID: #{component.componentId}</div>
                        <div className="mt-2 flex items-center">
                            <span className={`text-xs px-2 py-1 rounded-full ${
                            component.stock > 100 ? 'bg-green-100 text-green-800' :
                            component.stock > 50 ? 'bg-yellow-100 text-yellow-800' :
                            'bg-red-100 text-red-800'
                            }`}>
                ...
                {filteredCustomers.map((customer, index) => (
                    ...
                    <div className="flex items-start space-x-3">
                        ...
                        <div className="mt-2 flex flex-wrap gap-2">
                            ...
                            <span className="inline-flex items-center text-xs text-gray-500">
                            <Globe className="h-3 w-3 mr-1" /> {customer.countryName}
                            </span>
                            <span className="inline-flex items-center text-xs text-gray-500">
                            <MapPin className="h-3 w-3 mr-1" /> {customer.region}
                            </span>
                        </div>
                        </div>
                    </div>
            ...
            <div className="space-y-6">
                <div className="space-y-3">
                <h3 className="text-lg font-medium text-gray-900 px-1">Country Deliveries</h3>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    ...
                    <div key={index} className="bg-white rounded-lg shadow-sm p-4">
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            <h4 className="font-medium text-gray-900">{delivery.countryName}</h4>
                        </div>
                ...
                <div className="space-y-3">
                <h3 className="text-lg font-medium text-gray-900 px-1">Region Deliveries</h3>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    ...
                    <div key={index} className="bg-white rounded-lg shadow-sm p-4">
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            <h4 className="font-medium text-gray-900">{delivery.region}</h4>
                        </div>
            ...
            <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => setFilters({...filters, processStatus: e.target.value || undefined})}
                        className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                      >
                        <option value="">All Statuses</option>
                    ...
                    <div className="sm:col-span-2">
                      ...
                      <div className="mt-1 flex flex-col sm:flex-row gap-2">
                        <input
                          type="date"
                          value={filters.deliveryDateStart?.toISOString().split('T')[0] || ''}
                          onChange={(e) => setFilters({
                            ...filters, 
                            deliveryDateStart: e.target.value ? new Date(e.target.value) : undefined
                          })}
                        ...
                        <input
                          type="date"
                          value={filters.deliveryDateEnd?.toISOString().split('T')[0] || ''}
                          onChange={(e) => setFilters({
                            ...filters, 
                            deliveryDateEnd: e.target.value ? new Date(e.target.value) : undefined
                          })}
                          className="block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                        />
                      </div>
              ...
              <div className="space-y-3">
                {filterOrders.map((order) => (
                  <div key={order.number} className="bg-white rounded-lg shadow-sm p-4">
                    <div className="flex justify-between items-start">
                      <div>
                        <h4 className="font-medium text-gray-900">Order #{order.number}</h4>
                        <p className="text-sm text-gray-500 mt-1">{order.customerName}</p>
                        <div className="mt-2 flex flex-wrap items-center gap-2">
                          <span className={`text-xs px-2 py-1 rounded-full ${
                            order.paymentStatus === 'Ödendi' ? 'bg-green-100 text-green-800' :
                            order.paymentStatus === 'Kısmi Ödendi' ? 'bg-yellow-100 text-yellow-800' :
                            'bg-red-100 text-red-800'
                          }`}>
                            {order.paymentStatus}
                          </span>
                      ...
                      <div className="text-right">
                        <div className="font-semibold text-gray-900">
                          {order.totalPrice.toLocaleString()} {order.currencyType}
                        </div>
